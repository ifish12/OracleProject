...
public class CoursesArrayAdapter extends ArrayAdapter<Course> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(note.getTitle().length() <= 20)
            descriptionTextView.setText(note.getTitle()); // Comment has less than 20 characters and we should display entire thing
        else
            descriptionTextView.setText(note.getTitle().substring(0, 20)); // Just get first 20 characters of comment to display
